...
class KDTree
{
  ...
  public:
    ...
    typename KDTree<Dim>::KDTreeNode* buildKDTree(vector<Point<Dim>>& points, int left, int right, int currDim);
    ...
    typename KDTree<Dim>::KDTreeNode* copyNode(const KDTreeNode* node);
    ...
    void findNearestNeighborHelper(const KDTreeNode* node, const Point<Dim>& query,
                                             int level, Point<Dim>& nearest, double& distance) const;

    int distanceSquared(const Point<Dim>& first,
